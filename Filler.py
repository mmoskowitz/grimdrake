...
class Filler:
    ...
    def getsearchletters(self, search, length, index):
        if not (search in self.searchletters):
            self.searchletters[search] = {}  # create caching hash
        if index in self.searchletters[search]:
    ...
    def fill(self):
        ...
        while wlist:
            # find best
            wtuple = tuple(wlist)
            if wtuple in self.searchchoices:
                ...
                if not (self.full_auto):
                    word = input(
                        "type word to choose, [RET] for auto, '!' for full auto or '0' to skip: "
                    )
                if word and word in "!":
                    self.full_auto = True
            ...
            self.entrylist.append(
                (shortkey[2], self.numbers[(shortkey[0], shortkey[1])], word)
            )
