...
class Gui(Frame):  
    ...
    def createGridDisplay(self, width, height):
        ...
        for x in range(0, width - 1):
             for y in range(0, width):
                 vbar = Canvas(self.gridFrame, bg="#000", 
                               width=self.BAR_THICKNESS, height=self.BAR_LENGTH )
        ...
        for x in range(0, width):
             for y in range(0, width - 1):
                 hbar = Canvas(self.gridFrame, bg="#000", 
                               width=self.BAR_LENGTH, height=self.BAR_THICKNESS)
        ...
        for x in range(0, width - 1):
            for y in range(0, width - 1):
                corner = (Canvas(self.gridFrame, bg="#000",
                                 width=self.BAR_THICKNESS, height=self.BAR_THICKNESS))
    ...
    def displayGrid(self):
        ...
        infoText = "%s = GOOD %s OK %s LONG %s : len %s unc %s cns %s chk %s fil %s" % (puzzle.grid.gridscore(), scores[Grid.GOOD], scores[Grid.OKAY], scores[Grid.LONG], scores[Grid.LENGTH], scores[Grid.UNCHCOUNT], scores[Grid.CONSECUTIVE], scores[Grid.CHECKED], scores[Grid.FILLED])
    ...
    def createControls(self):
        ...
        self.randomizeButton = Button(self.controlFrame, text="Randomize", command=self.randomizeGrid)
        ...
        self.doubleBarButton = Button(self.controlFrame, text="Double Bars", command=self.doubleBarSteps)
