...
class Grid:
    ...
    def isblack(self, column, row):
        return self.isunch(column, row, Grid.ACROSS) and self.isunch(
            column, row, Grid.DOWN
        )
    ...
    def gridscore(self):
        self.generatescore()
        scores = self.scores
        score = sum([scores[key] * Grid.VALUES[key] for key in list(scores.keys())])
        return score
    ...
    def reverse_line(self, line, tl):
        line = ((line >> 1) & 0x5555555555555555) | ((line << 1) & 0xAAAAAAAAAAAAAAAA)
        line = ((line >> 2) & 0x3333333333333333) | ((line << 2) & 0xCCCCCCCCCCCCCCCC)
        line = ((line >> 4) & 0x0F0F0F0F0F0F0F0F) | ((line << 4) & 0xF0F0F0F0F0F0F0F0)
        line = ((line >> 8) & 0x00FF00FF00FF00FF) | ((line << 8) & 0xFF00FF00FF00FF00)
        line = ((line >> 16) & 0x0000FFFF0000FFFF) | ((line << 16) & 0xFFFF0000FFFF0000)
        line = ((line >> 32) & 0x00000000FFFFFFFF) | ((line << 32) & 0xFFFFFFFF00000000)
        line = line >> (64 - tl)
        return line
    ...
    def barpost(self, line, space, dir):
        ...
        return space == self.dimension[dir] - 1 or self.bar_in(
            self.bars[dir][line], space, self.dimension[dir]
        )

    def barpre(self, line, space, dir):
        return space == 0 or self.bar_in(
            self.bars[dir][line], space - 1, self.dimension[dir]
        )
    ...
    def isnumber(self, column, row):
        for dir in (Grid.ACROSS, Grid.DOWN):
            if self.barbefore(column, row, dir) and not (
                self.barafter(column, row, dir)
            ):
                return True
        return False
